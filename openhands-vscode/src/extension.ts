...
function startOpenHandsInTerminal(options: { task?: string; filePath?: string }): void {
  try {
    ...
    if (options.filePath) {
      ...
      // Sanitize task string for command line to prevent unexpected shell expansions.
    ...
    console.error('[OpenHands Extension] Error in startOpenHandsInTerminal:', error);
    vscode.window.showErrorMessage('OpenHands: Failed to start session in terminal. See developer console for details.');
  }
}
