...
suite('OpenHands Extension Integration Tests', () => {
	...
	test('Extension should activate successfully', async () => {
		// Get the extension
		const extension = vscode.extensions.getExtension('openhands.openhands-tab-extension');
	...
	test('Status message flow integration test', async () => {
		...
		assert.ok(extension?.isActive, 'Extension should be active for status message handling');
	});
});
